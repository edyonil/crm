    ...
    <nav role="navigation" aria-label="{{ __('Pagination Navigation') }}">
        ...
        <div class="grid px-4 py-3 text-xs font-semibold tracking-wide text-gray-500 uppercase border-t dark:border-gray-700 bg-gray-50 sm:grid-cols-9 dark:text-gray-400 dark:bg-gray-800">
            <div class="flex items-center col-span-3">
                <p class="text-xs xs:text-sm text-gray-900">Exibindo 
                <span class="font-medium">{{ $paginator->firstItem() }}</span> 
                de <span class="font-medium">
                {{ $paginator->lastItem() }}</span> para <span></span><span class="font-medium">{{ $paginator->total() }}</span> resultados</p>
            </div>
                            ...
                            @foreach ($element as $page => $url)
                                @if ($page == $paginator->currentPage())
                                    <span aria-current="page">
                                        <span class="relative inline-flex items-center px-4 py-2 -ml-px text-sm font-medium text-white bg-white border border-purple-600 cursor-default leading-5 bg-purple-600 transition-colors duration-150">
                                            {{ $page }}
                                        </span>
                                    </span>
                                @else
